...
public class TaskService {
  ...
  public Optional<TaskDescription> uploadTask(MultipartFile taskArchive)
    ...
    // create file
    try {
      fileStorageHandler.createFile(
          storage, new FileData(uploadTaskArchive.getDescriptorFile(), "descriptor.yml"));
    ...
    try {
      ...
      try {
        logger.debug(
            "UploadTask : failed to push image to registry, attempting to delete storage...");
        fileStorageHandler.deleteStorage(storage);
        logger.info("UploadTask : storage deleted");
  ...
  private TaskIdentifiers generateTaskIdentifiers(UploadTaskArchive uploadTaskArchive) {
    ...
    String imageRegistryCompliantName =
        imageIdentifierFromDescriptor.replace(".", "/") + ":" + version;

    return new TaskIdentifiers(taskLocalIdentifier, storageIdentifier, imageRegistryCompliantName);
  }

  public FileData retrieveYmlDescriptor(String namespace, String version)
  ...
  public FileData retrieveYmlDescriptor(String id)
      throws TaskServiceException, TaskNotFoundException {
    logger.info("Storage : retrieving descriptor.yml...");
  ...
  private void createRunStorages(UUID taskRunID) throws RunTaskServiceException {
    ...
    try {
      ...
      logger.error(
          "tasks/{namespace}/{version}/runs : failed to create storage [" + e.getMessage() + "]");
